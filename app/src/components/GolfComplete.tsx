...
export default function GolfComplete({
  ...
  const ratingColor = underPar ? 'text-[#a6e3a1]' : onPar ? 'text-[#89b4fa]' : 'text-[#f9e2af]'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      ...
      <div className={`bg-[#1e1e2e] border border-[#313244] rounded-2xl p-8 max-w-sm w-full mx-4 text-center relative z-10 transition-all duration-500 ${showContent ? 'scale-100 opacity-100' : 'scale-90 opacity-0'}`}>
        ...
        {isNewBest && (
          <p className="text-xs text-[#cba6f7] font-medium mb-3">New Personal Best!</p>
        )}
        ...
        <div className="bg-[#313244] rounded-lg px-4 py-3 mb-6">
          <div className="flex items-center justify-center gap-4">
            <div className="text-center">
              <div className="text-xs text-[#6c7086]">Your Blocks</div>
              <div className={`text-2xl font-bold ${ratingColor}`}>{blockCount}</div>
            </div>
            <div className="text-[#6c7086] text-lg">vs</div>
            <div className="text-center">
              <div className="text-xs text-[#6c7086]">Par</div>
              <div className="text-2xl font-bold text-[#89b4fa]">{par}</div>
            </div>
          </div>
          {overPar && (
            <p className="text-xs text-[#f9e2af] mt-2">Try again with fewer blocks to reach par!</p>
          )}
        </div>
