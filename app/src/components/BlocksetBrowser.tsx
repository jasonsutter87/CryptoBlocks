    ...
    <div className="flex-1 overflow-auto bg-base p-4 md:p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6 md:mb-8">
          <div>
            <h2 className="text-xl md:text-2xl font-bold text-text">Blocksets</h2>
            <p className="text-xs md:text-sm text-overlay mt-1">Step-by-step guided tutorials — like LEGO instruction sets</p>
          </div>
          <div className="flex items-center gap-3 md:gap-4">
            <button
              onClick={() => setShowStats((v) => !v)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-colors ${
                showStats ? 'bg-success text-base' : 'bg-surface-0 text-text hover:bg-surface-1'
              }`}
            >
        ...
        {showStats && (
          <div className="mb-6 rounded-xl border border-surface-0 bg-mantle p-4 md:p-5">
            ...
            <div className="grid grid-cols-3 gap-3 md:gap-4 mb-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-accent">{totalBlocksets - completedCount}</div>
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      <div className="text-sm font-medium text-text">{completed}/{pack.blocksets.length}</div>
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-surface-0/50 cursor-pointer ${
                            i < pack.blocksets.length - 1 ? 'border-b border-surface-0/50' : ''
                          }`}
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-medium text-text truncate">{blockset.title}</span>
                              <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${difficultyColor(blockset.difficulty)}`}>
                                {blockset.difficulty}
                              </span>
                              <span className="text-[10px] text-overlay">~{blockset.estimatedMinutes}min</span>
                            </div>
                          </div>
