...
export default function CodeToBlocksModal({ onConvert, onClose }: CodeToBlocksModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-base border border-surface-0 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col mx-4">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-surface-0">
          <div>
            <h2 className="text-xl font-bold text-text">Code to Blocks</h2>
            <p className="text-sm text-overlay mt-0.5">Paste JavaScript — get snapped-together blocks</p>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Editor */}
        <div className="flex-1 min-h-0 border-b border-surface-0">
          {monacoFailed ? (
            <textarea
              value={code}
              onChange={(e) => setCode(e.target.value)}
              className="w-full h-[400px] bg-base text-text text-[13px] font-mono p-3 resize-none outline-none"
              spellCheck={false}
            />
          ) : (
            <Editor
              ...
              theme="vs-dark"
              loading={
                <div className="h-[400px] flex items-center justify-center bg-base text-overlay text-sm">
                  Loading editor...
                </div>
              }
        ...
        {error && (
          <div className="px-6 py-3 bg-danger/10 border-b border-danger/30">
            <p className="text-sm text-danger font-medium">Parse Error</p>
            <p className="text-sm text-danger/80 mt-0.5">{error}</p>
          </div>
        )}
        {warnings.length > 0 && (
          <div className="px-6 py-3 bg-warn/10 border-b border-warn/30 max-h-24 overflow-auto">
            <p className="text-sm text-warn font-medium">Warnings</p>
            {warnings.map((w, i) => (
              <p key={i} className="text-sm text-warn/80 mt-0.5">- {w}</p>
            ))}
          </div>
        )}
