  ...
  return (
    <div className="px-6 py-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-semibold text-overlay uppercase tracking-wider">Student Projects</h3>
        <button
          onClick={handleUpload}
          className="px-3 py-1.5 text-xs font-bold text-base bg-success hover:bg-success/80 rounded-lg"
        >
          Upload from Editor
        </button>
      </div>
      {classroom.projects.length === 0 ? (
        <p className="text-sm text-overlay italic">No projects shared yet.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {classroom.projects.map((p) => (
            <div key={p.id} className="bg-base rounded-lg px-4 py-3">
